...
export default function Header({ theme = "dark" }) {
  ...
  const [showGoldPlanDropdown, setShowGoldPlanDropdown] = useState(false);
  const [showGoldPlanMobileDropdown, setShowGoldPlanMobileDropdown] = useState(false);
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      ...
      if (showGoldPlanDropdown && !e.target.closest('.goldplan-dropdown')) {
        setShowGoldPlanDropdown(false);
      }
    };

    document.addEventListener('click', handleClickOutside);
    return () => document.removeEventListener('click', handleClickOutside);
  }, [showLanguageDropdown, showServicesDropdown, showJoinUsDropdown, showGoldPlanDropdown]);
    ...
    <>
      <nav className={`w-[96%] max-w-[1400px] ${navBgColor} rounded-full px-4 md:px-5 py-2.5 flex justify-between items-center shadow-xl fixed top-4 left-1/2 transform -translate-x-1/2 z-50 mx-auto`}>
        ...
        <div className={`hidden lg:flex items-center gap-6 ${textColor} text-[15px] font-medium tracking-wide`}>
          ...
          <div
            className="relative goldplan-dropdown"
            onMouseEnter={() => setShowGoldPlanDropdown(true)}
            onMouseLeave={() => setShowGoldPlanDropdown(false)}
          >
            <button className={`transition flex items-center gap-2 ${showGoldPlanDropdown ? (isLightTheme ? 'text-[#2E3B78]' : 'text-[#FCD64C]') : servicesHoverColor}`}>
              {t('header.goldSubscriptionPlan')}
              <svg
                className={`w-3 h-3 transition-transform duration-200 ${showGoldPlanDropdown ? 'rotate-180' : ''}`}
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" />
            ...
            {showGoldPlanDropdown && (
              <div className="absolute top-full right-0 pt-2 bg-transparent min-w-[160px] z-50">
                <div className="bg-white rounded-xl shadow-2xl overflow-hidden">
                  ...
                  <Link
                    href={`${getLocalePath('/pages/usa-dropshipping', pathname)}#pricing`}
                    className={`block px-4 py-3 text-[#2E3B78] ${dropdownHoverColor} transition text-sm font-medium`}
                    onClick={() => setShowGoldPlanDropdown(false)}
                  >
                    {t('header.usaPlan')}
      ...
      {showMobileMenu && (
        <div className={`lg:hidden fixed top-20 left-1/2 transform -translate-x-1/2 w-[96%] max-w-[1400px] ${isLightTheme ? 'bg-[#E8F0FE]' : 'bg-[#2D3E7E]'} rounded-2xl shadow-2xl z-40 mt-2 overflow-hidden`}>
          <div className="flex flex-col p-6 space-y-4">
            ...
            <div className="relative goldplan-dropdown">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setShowGoldPlanMobileDropdown(!showGoldPlanMobileDropdown);
                }}
                className={`w-full flex items-center justify-between ${isLightTheme ? 'text-[#2E3B78]' : 'text-white'} text-[15px] font-medium py-3 ${isLightTheme ? 'hover:text-[#2E3B78]' : 'hover:text-[#FCD64C]'} transition`}
              >
                <span>{t('header.goldSubscriptionPlan')}</span>
                <svg
                  className={`w-4 h-4 transition-transform ${showGoldPlanMobileDropdown ? "rotate-180" : ""}`}
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
                  <path
